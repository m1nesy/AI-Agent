...
class RagService:
    ...
    def collection_name(self):
        return os.getenv("KB_COLLECTION_NAME", "dw_ai")

    @property
    def project_name(self):
        return os.getenv("KB_PROJECT_NAME", "default")

    @property
    def account_id(self):
        return os.getenv("VOLC_ACCOUNT_ID", "")
    ...
    async def retrieve(self, query: str) -> str:
        ...
        if not self.ak or not self.sk or not self.account_id:
            print(f"[WARN] [RagService] 配置缺失: 请检查 VOLC_AK, VOLC_SK, VOLC_ACCOUNT_ID(当前: {self.account_id})")
            return ""

        path = "/api/knowledge/collection/search_knowledge"

        print(f"[RAG] 查询参数: project={self.project_name}, name={self.collection_name}", flush=True)
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f"[ERROR] [RagService] 请求失败: {resp.status_code}, {resp.text}")
                return ""
            ...
            if not contents:
                print(f"[WARN] [RagService] 未检索到匹配的知识内容")
                return ""
            ...
            print(f"[OK] [RagService] 成功提取 {len(contents)} 条知识内容, 共 {len(context_text)} 字符")
            return context_text


        except Exception as e:
            print(f"[ERROR] [RagService] 异常: {e}")
            return ""
